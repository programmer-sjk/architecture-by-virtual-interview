...
## 12장. 채팅 시스템 설계
- 채팅 서비스의 경우 어떤 프로토콜을 사용하는지도 중요한 문제이다. 채팅 시스템도 클라이언트가 메시지를 채팅 서비스에 보낼 떄 오랫동안 검증된
HTTP 프로토콜을 사용한다. 하지만 서버에서 클라이언트로 임의 시점에 메시지를 보내는데는 쉽게 쓰일 수 없다.
- 서버가 연결을 만드는 것처럼 동작하기 위해 많은 기법이 제안되어 왔는데 폴링, 롱 폴링, 웹 소켓 등의 기술이 있다.

### 폴링
- 클라이언트가 주기적으로 서버에게 새 메시지가 있냐고 물어보는 방법. 
- 폴링을 자주할수록 비용이 올라가며, 답해줄 메시지가 없는경우 서버자원이 불필요하게 낭비된다는 문제 존재.

### 롱 폴링
- 폴링은 여러가지로 비 효율적이라 나온 기법이다.
- 롱 폴링의 경우 새 메시지가 반환되거나 타임아웃 될 떄까지 연결을 유지한다.
- 클라이언트는 새 메시지를 받으면 기존 연결을 종료하고 서버에 새로운 요청을 한다.
- 서버 입장에서 클라이언트가 연결을 해제했는지 알 수 있는 좋은 방법이 없고 여전히 타임아웃이 일어날때마다 서버의 자원이 낭비된다.

### 웹소켓
- 서버가 클라이언트에게 비동기 메시지를 보낼 때 가장 널리 사용하는 기술
- 웹 소켓 연결은 클라이언트가 시작하며 양방향이다. 이 연결은 처음에는 HTTP 연결이지만 특정 핸드쉐이크 절차를 거쳐 웹 소켓 연결로 업그레이드 된다.
- 이 웹 소켓을 통해 서버는 클라이언트에게 비동기로 메시지를 전송할 수 있다.

### 저장소
- 채팅 시스템에서 다루는 데이터는 보통 두 가지다. 하나는 사용자 프로필, 설정, 친구목록 같은 일반 데이터로 안정성을 보장하는 RDBMS에 저장한다.
- 두번째 데이터는 채팅 이력이다. 채팅 이력 데이터의 양은 엄청나게 많으며 빈번하게 사용되는 것은 최근에 주고받은 메시지다. 
- 사용자가 보통 최근 데이터를 보는건 사실이지만 검색 기능이나 특정 사용자가 언급된 메시지를 보려면 무작위적으로 데이터 접근이 필요하다.
- 이 설계에서 키-값 저장소를 추천하는데 이유는 아래와 같다.
  - 키-값 저장소는 수평적 규모 확장이 쉽다.
  - 키-값 저장소는 데이터 접근 지연시간이 낮다.
  - 이미 안정적인 채팅 시스템이 HBase, 카산드라 등의 키-값 저장소를 채택하고 있다.
